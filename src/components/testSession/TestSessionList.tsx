  ...
  return (
    <div id="test-session-list-view" className="p-3 sm:p-5 md:p-6 lg:p-8 space-y-4 sm:space-y-6 max-w-7xl mx-auto">
      ...
      {(() => {
        ...
        const items = activeSession.smartTestPlan?.items.filter(
          (i) => i.executionStatus !== 'NOT_APPLICABLE'
        ) || [];
        const completedItems = items.filter((i) => i.executionStatus === 'COMPLETED');
        const nextItem =
          items.find((i) => i.executionStatus === 'READY' || i.executionStatus === 'IN_PROGRESS') ||
          items.find((i) => i.executionStatus !== 'COMPLETED');
          ...
          <div className="p-4 sm:p-5 bg-gradient-to-r from-slate-900 to-indigo-950 text-white rounded-2xl shadow-md border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4 animate-in fade-in">
            <div className="space-y-1">
              ...
              <h3 className="text-base font-bold text-white">
                {activeSession.instrumentSnapshot.manufacturer} {activeSession.instrumentSnapshot.model}
              </h3>
              <div className="text-xs text-slate-300 flex flex-wrap items-center gap-x-3 gap-y-1 pt-0.5">
                <span>
                  Progress: <strong className="text-white">{completedItems.length} of {items.length}</strong> tests completed
                </span>
                {lastCompleted && (
                  <span>
                    • Last completed: <strong className="text-emerald-300">{lastCompleted.testName}</strong>
                  </span>
                )}
                {nextItem && (
                  <span>
                    • Next: <strong className="text-amber-300">{nextItem.testName}</strong>
                  </span>
                )}
              </div>
            </div>
