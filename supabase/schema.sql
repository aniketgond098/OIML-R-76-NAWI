...
-- Allow authenticated and anon access with appropriate permissions for metrology operations
DROP POLICY IF EXISTS "Allow public read on laboratories" ON laboratories;
DROP POLICY IF EXISTS "Allow authenticated upsert on laboratories" ON laboratories;
...
DROP POLICY IF EXISTS "Allow upsert on users" ON users;
DROP POLICY IF EXISTS "Allow all on users" ON users;
CREATE POLICY "Allow all on users" ON users FOR ALL USING (true) WITH CHECK (true);

DROP POLICY IF EXISTS "Allow read on instruments" ON instruments;
DROP POLICY IF EXISTS "Allow upsert on instruments" ON instruments;
...
DROP POLICY IF EXISTS "Allow read on equipment" ON equipment;
DROP POLICY IF EXISTS "Allow upsert on equipment" ON equipment;
...
DROP POLICY IF EXISTS "Allow read on test_sessions" ON test_sessions;
DROP POLICY IF EXISTS "Allow upsert on test_sessions" ON test_sessions;
...
DROP POLICY IF EXISTS "Allow upsert on reports" ON reports;
...
DROP POLICY IF EXISTS "Allow read on attachments" ON attachments;
DROP POLICY IF EXISTS "Allow upsert on attachments" ON attachments;
...
-- ==============================================================================
-- 13. RELAX STRICT FOREIGN KEYS (FOR OFFLINE / MULTI-DEVICE CLIENT SYNC)
-- ==============================================================================
...
-- ==============================================================================
-- 14. PUBLIC SECURE VERIFICATION RPC (Zero-Trust Public Status Lookup)
-- ==============================================================================
...
DECLARE
  ...
  v_result JSONB;
BEGIN
  -- 1. Find instrument by public_verification_id or id
  ...
  -- 2. Check if QR verification is explicitly revoked/disabled
  ...
  -- 3. Fetch latest finalized & approved report
  ...
  -- 4. Check for active/pending test session
  ...
  -- 5. Determine public status strictly
  ...
  -- 6. Collect non-sensitive public history
  ...
  -- 7. Build sanitized public response
